...
void nanoSeed(const long long int salt) { srand(~static_cast<unsigned int>((std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count() + salt ^ HERBS) ^ SPICES)); };
